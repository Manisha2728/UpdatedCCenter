...
class DocumentSearchLiveFeeds(PageBaseModel):
...
class DocumentSearchLiveFeedsProperties(ConfigurationEntityBaseModel):
    parent =                models.ForeignKey('DocumentSearchLiveFeeds', on_delete=models.SET_NULL, null=True, default=1, editable=False)
    frequency_mode   =        models.IntegerField(verbose_name='Live-feed frequency mode:', choices=FREQUENCY_MODE, default=1, help_text='Each insert, update, and delete of a clinical document in the CDR triggers queuing the document in an operational table for Elasticsearch indexing. Specify how often the indexing occurs.<br/>* <b>Continuous</b><br/>* <b> Time frame:</b><br/><i>Default: Continuous.</i>')
    start_scheduled_time       =        models.TimeField(verbose_name="Live-feed start time", blank = True, default = None, null = True, help_text='Enter a time in format HH:MM:SS, click Now, or click the clock icon to select from a list of start times.<br/><i>Default: Empty</i>')
    end_scheduled_time         =        models.TimeField(verbose_name="Live-feed end time", blank = True, default = None, null = True, help_text='Enter a time in format HH:MM:SS, click Now, or click the clock icon to select from a list of start times.<br/><i>Default: Empty</i>')
    ...
    protected_systems_rate2    = models.IntegerField(verbose_name='Group #2 aggregated rate (req/min)', validators=[validators.MinValueValidator(1), validators.MaxValueValidator(sys.maxsize)], blank=True, null=True, help_text=get_help_text('''
        Define the aggregated rate of content requests to systems from group. If the rate is over 2000, the group becomes an unprotected source and is threaded together with other sources (that is, not a special group or thread for those systems).		
    ''','Empty'))
    unprotected_systems_rate    = models.IntegerField(verbose_name='Standard aggregated rate (req/min)', default=2000, validators=[validators.MinValueValidator(1), validators.MaxValueValidator(sys.maxsize)], blank=False, null=False, help_text=get_help_text('''
